      ...
      do {
        const { data } = await axios.post(
          "https://app.adquadrant.com/Extjs.ashx?s=reportleadsbyaff",
          {
            ...
            report_id: 68,
            date_range: "custom",
            // start_date: moment("2024-01-01").format("M/DD/YYYY"),
            // end_date: moment().format("M/DD/YYYY"),
        ...
        if (data?.rows)
          for (let row of data?.rows) {
            ...
            const {data: response} = await axios.post(
              "https://app.adquadrant.com/store.ashx?s=leadvertinfo",
              {
                lid: lid,
              },
              {
            ...
            let debtAmount = response?.rows?.find(
              (row, idx) => row.field_name == "debt_amount"
            );
            let pageUrl = response?.rows?.find(
              (row, idx) => row.field_name == "page"
            );
            rows.push({
              ...
              subid_4: row.subid_4,
              subid_5: row.subid_5,
              page: pageUrl ? pageUrl.value : "",
              debt_amount: debtAmount ? debtAmount.value : "",
              associated_revenue: row.price_received,
              date_created: moment(row.lead_date).format("YYYY-MM-DD") + ""
            });
          }
      } while (p < total);
      console.log(rows);
